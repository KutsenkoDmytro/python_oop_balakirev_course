...
'''
#СОЗДАНИЕ ГЛУБОКОЙ КОПИИ ЭКЗЕМПЛЯРА КЛАССА
...
class Box:
    ...
    def _get_total_weight(self):
        return sum(obj[1] for obj in self._things)

    def add_thing(self, obj):
        if obj[1] + self._get_total_weight() > self._max_weight:
            raise ValueError('превышен суммарный вес вещей')
...
class BoxDefender:
    ...
    def __enter__(self):
        self.__temp = copy.deepcopy(self.__b)
        return self.__temp

    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type is None:
            self.__b._things[:] = self.__temp._things
...
box.add_thing(("спички", 46.6))
box.add_thing(("рубашка", 134))
print(box._things, id(box))

with BoxDefender(box) as b:
    b.add_thing(("зонт", 346.6))
    b.add_thing(("шина", 400))
