...
Подвиг 5. Вы начинаете разрабатывать свой сервис по тестированию. Для этого вам поручается разработать базовый класс Test для всех видов тестов, объекты которого создаются командой:
...
Sample Input:Какое значение получим, при выполнении команды int(5.7)? | 56
Sample Output:False
...
class Test:
    ...
    def descr(self, value):
        if not (isinstance(value, str) and \
                self.__min_char <= len(value) <= self.__max_char):
            raise ValueError(
...
class TestAnsDigit(Test):
    ...
    def __setattr__(self, key, value):
        ...
        if key == '_ans_digit' and not (type(value) in (int, float)):
            raise ValueError('недопустимые значения аргументов теста')

        if key == '_max_error_digit' and not (
    ...
    def run(self):
        '''Осуществляет проверку на вхождение в диапазон.'''
        ans = float(input())

        min_val = self._ans_digit - self._max_error_digit
        max_val = self._ans_digit + self._max_error_digit
        return min_val <= ans <= max_val
