...
#ЛОГИРОВАНИЕ МЕТОДОВ КЛАССА С ПОМОЩЬЮ ДЕКОРАТОРОВ


vector_log = []

def add_method(func, lst):
    '''Декоратор который выполняет логирование вызваных методов.'''
...
def class_log(lst):
    '''Декоратор который оборачивает все методы класса (первый параметр - список в кторый будут сохнарены вызваные методы.)'''
    def handler(cls):
        methods = {k: v for k, v in cls.__dict__.items() if callable(v)}
        for k, v in methods.items():
            setattr(cls, k, add_method(v,lst))
        return cls
    return handler
