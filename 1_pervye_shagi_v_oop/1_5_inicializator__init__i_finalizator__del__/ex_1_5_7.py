...
Обратите внимание при создании объекта класса MotherBoard можно передавать несколько
объектов класса Memory, максимум N - по числу слотов памяти на материнской плате (N = 4).
...
class MotherBoard:
    ...
    def get_config(self):
        config = [f'Материнская плата: {self.name}',
                  ...
                  f'Память: {"; ".join([f"{obj.name} - {obj.volume}" for obj in self.mem_slots])}',
                  ]
        return config

cpu = CPU('AMD Ryzen', 5)
mem_1 = Memory('Mem_1', 500)
mem_2 = Memory('Mem_2', 1000)


mb = MotherBoard('New mother board',cpu,mem_1,mem_2)
