...
В классе Product через магические методы (подумайте какие) осуществить проверку на тип присваиваемых данных локальным атрибутам объектов класса (например,
id - целое число, name - строка и т.п.). Если проверка не проходит, то генерировать исключение командой:
...
class Shop:
    '''Клас для управления магазином в целом.'''
...
class Product:
    '''Клас для представления отдельного товара.'''
    LAST_ID = 0
    ...
    def __setattr__(self, key, value):
        if key in {'weight', 'price'} and type(value) in {int, float} or \
                key == 'name' and type(value) is str or \
                key == 'id' and type(value) is int:
    ...
    def __delattr__(self, item):
        if item != 'id':
            ...
            raise ValueError('Атрибут id удалять запрещено.')
