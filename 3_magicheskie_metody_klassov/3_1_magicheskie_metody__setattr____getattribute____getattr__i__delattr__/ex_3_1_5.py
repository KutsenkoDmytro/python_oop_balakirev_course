...
class Course:
    '''Отвечает за управление курсом в целом.'''
...
class Module:
    '''Описывает один модуль (раздел) курса.'''
...
class LessonItem:
    ...
    def __setattr__(self, key, value):
        if key == 'title' and type(value) is str or \
                key in {'practices', 'duration'} and type(value) is int:
    ...
    def __delattr__(self, item):
        if item not in {'title', 'practices', 'duration'}:
            ...
            raise ValueError(f'Атрибут {item} удалять запрещено.')
