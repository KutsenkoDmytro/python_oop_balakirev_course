...
class LoginForm:
    '''Класс для обработки формы авторизации на стороне сервера.'''
    ...
    def is_validate(self):
        ...
        for v in self.validators:
            if not (v(self.login) and v(self.password)):
                return False
...
class LengthValidator:
    '''Функтор проверяющий длинну переданной строки.'''
    ...
    def __call__(self, *args, **kwargs):
        if type(args[0]) is str:
            ...
            raise ValueError('ожидается аргумент типа str')


class CharsValidator:
    '''Функтор проверяющий переданную строку на корректность елементов.'''
