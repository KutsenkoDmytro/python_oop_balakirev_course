...
class ListMath:
    ...
    def __setattr__(self, key, value):
        "Перед установкой значений атрибутов экземпляров делает проверку."
        if key == 'lst_math':
            value = self.__get_lst_crt_val(value)
        object.__setattr__(self, key, value)

    def __get_lst_crt_val(self, lst):
        '''Формирует список с элементами корректного типа.'''
        if type(lst) is list:
