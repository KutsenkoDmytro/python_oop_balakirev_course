...
class FileAcceptor:
    ...
    def __init__(self, *args):
        lst = args if args else []
        self.f_extensions = lst

    def __call__(self, file, *args, **kwargs):
        return file[file.rfind('.') + 1:] in self.f_extensions

    def __add__(self, other):
        if isinstance(other, self.__class__):
            res = self.__class__(*set(self.f_extensions + other.f_extensions))
            return res

# # Для проверки.
# filenames_ = ["boat.jpg", "ans.web.png", "text.txt", "www.python.doc", "my.ava.jpg", "forest.jpeg", "eq_1.png", "eq_2.xls"]
# acceptor = FileAcceptor('jpg', 'bmp', 'jpeg')
# filenames = list(filter(acceptor, filenames_))
# print(filenames)
#
# acceptor_images = FileAcceptor("jpg", "jpeg", "png")
# acceptor_docs = FileAcceptor("txt", "doc", "xls")
# filenames = list(filter(acceptor_images + acceptor_docs, filenames_))
# print(filenames)
