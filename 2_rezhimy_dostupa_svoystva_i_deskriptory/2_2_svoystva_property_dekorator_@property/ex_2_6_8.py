...
проходит, то осуществляется переход к следующему объекту по левой стрелке (с единицей),
...
1 - да, 0 - нет. Каждый элемент этого списка соответствует своему вопросу (своей вершине дерева).
...
С ней связан второй элемент вектора x со значением 0, следовательно, мы переходим по правой ветви и попадаем в вершину "будет кодером".
...
'''
class TreeObj:
    '''Описывает вершини и листья решающего дерева.'''
...
class DecisionTree:
    '''Описывает работу дерева вцелом.'''
    root = None
    ...
    def predict(cls, root, x):
        ...
        #     if v == 1 and root.left != None:
        #         root = root.left
        #     elif v == 0 and root.right != None:
        #         root = root.right
        #     else:
        #         break
        # return root.value
