...
class Point:
    '''Описывает координаты точки на плоскости.'''
    ...
    def __check_coords(cls, x, y):
        return {type(x), type(y)} <= {int, float}
...
class Rectangle:
    ...
    def __init__(self, *args):
        match args:
            ...
            case pt_1, pt_2 if type(pt_1) is Point and type(pt_2) is Point:
                self.__sp = pt_1
                self.__ep = pt_2
            case _:
                raise ValueError('неожиданное количество или тип данных')
...
rec = Rectangle(Point(0, 0), Point(20, 34))
